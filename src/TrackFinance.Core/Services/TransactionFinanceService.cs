...
public class TransactionFinanceService : ITransactionFinanceService
{
  ...
  public async Task<Result<List<TransactionDataDto>>> GetTransactionItemsByDatesAsync(int userId, TransactionType transactionType, DateTime initDate, DateTime endDate, CancellationToken cancellationToken = default)
  {
    var itemByDates = await _repository.ListAsync(new ItemsByDatesSpec(userId, transactionType , initDate, endDate), cancellationToken);
    return GetListTransactionByDate(itemByDates);
  }

  private static Result<List<TransactionDataDto>> GetListTransactionByDate(List<Transaction> itemsByDate)
  {
    var list = new List<TransactionDataDto>();

    var transactionsByDate = itemsByDate
        .Select(t => new
        {
          t.ExpenseDate.Year,
          t.ExpenseDate.Month,
        ...
        .GroupBy(t => new { t.Year, t.Month, t.Day, t.TransactionDescriptionType, t.TransactionType })
        .Select(g => new
        {
          g.Key.Year,
          g.Key.Month,
